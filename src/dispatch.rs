...
#[macro_export]
macro_rules! numpy_dispatch_quant {
    // single arg function
    ($py:ident, $f:ident, $ret_type:ty, $arr:ident) => {
        |x: &'a PyAny| -> $ret_type {
            if let Ok(i) = x.extract::<numpy::PyReadonlyArrayDyn<i8>>() {
    ...
    ($py:ident, $f:ident, $ret_type:ty, $arr1:ident, $arr2:ident) => {
        |x: &'a PyAny, y: &'a PyAny| -> $ret_type {
            ...
            if let (Ok(i), Ok(j)) = (
                x.extract::<numpy::PyReadonlyArrayDyn<f32>>(),
                y.extract::<numpy::PyReadonlyArrayDyn<f32>>(),
            ) {
                return $f::<f32>($py, i, j);
            }
            if let (Ok(i), Ok(j)) = (
                x.extract::<numpy::PyReadonlyArrayDyn<f64>>(),
                y.extract::<numpy::PyReadonlyArrayDyn<f64>>(),
            ) {
                return $f::<f64>($py, i, j);
            }
            Err(PyErr::new::<pyo3::exceptions::PyTypeError, _>(
    ...
    ($py:ident, $f:ident, $ret_type:ty, $arr1:ident, $arr2:ident, $arr3:ident) => {
        |x: &'a PyAny, y: &'a PyAny, z: &'a PyAny| -> $ret_type {
            ...
            if let (Ok(i), Ok(j), Ok(k)) = (
                x.extract::<numpy::PyReadonlyArrayDyn<f32>>(),
                y.extract::<numpy::PyReadonlyArrayDyn<f32>>(),
                z.extract::<numpy::PyReadonlyArrayDyn<f32>>(),
            ) {
                return $f::<f32>($py, i, j, k);
            }
            if let (Ok(i), Ok(j), Ok(k)) = (
                x.extract::<numpy::PyReadonlyArrayDyn<f64>>(),
                y.extract::<numpy::PyReadonlyArrayDyn<f64>>(),
                z.extract::<numpy::PyReadonlyArrayDyn<f64>>(),
            ) {
                return $f::<f64>($py, i, j, k);
            }
            Err(PyErr::new::<pyo3::exceptions::PyTypeError, _>(
                "Unsupported numpy dtype",
            ))
